...
from wsod.evaluation import (
    PascalVOCDetectionEvaluator,
    inference_on_dataset
)
from wsod.modeling import (
...
class Trainer(DefaultTrainer):
    ...
    def test_with_TTA(cls, cfg, model):
        ...
        if cfg.TEST.AUG.TYPE == "average":
            model = GeneralizedRCNNWSWithTTAAverage(cfg, model)
        elif cfg.TEST.AUG.TYPE == "union":
            model = GeneralizedRCNNWSWithTTAUnion(cfg, model)
        else:
            raise ValueError(
                "Cannot test with augmentation of type '{}'!"
                "Supported types are: 'average' or 'union'".format(cfg.TEST.AUG.TYPE)
            )
        evaluators = [
            cls.build_evaluator(
                cfg, name, output_folder=os.path.join(cfg.OUTPUT_DIR, "inference_TTA")
            )
            for name in (cfg.DATASETS.TEST + cfg.DATASETS.TRAIN)
        ]
