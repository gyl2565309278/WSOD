...
__all__ = ["pcl_inference", "PCLOutputLayers", "PCLROIHeads"]
...
def pcl_inference(
    ...
    """
    Call `pcl_inference_single_image` for all images.
...
def pcl_inference_single_image(
    boxes,
    scores,
    ...
    Args:
        Same as `pcl_inference`, but with boxes, scores, and image shapes
        per image.

    Returns:
        Same as `pcl_inference`, but for only one image.
    """
...
def _log_classification_stats(pred_logits, gt_classes, prefix="pcl"):
    """
...
class PCLOutputLayers(nn.Module):
    ...
    def losses(
        ...
        _log_classification_stats(scores, gt_classes, prefix="pcl_r{}".format(self.refine_k))
        ...
        if not self.use_pcl_loss:
            ...
            loss_cls = torch.sum(gt_scores * loss_clss) / N
        else:
...
class PCLROIHeads(WeakROIHeads):
    ...
    def _init_box_head(cls, cfg, input_shape):
        ...
        for k in range(1, refine_K + 1):
            box_predictor.append(PCLOutputLayers(cfg, box_head.output_shape, k, refine_reg[k - 1]))
    ...
    def _forward_box(
        ...
        if self.training:
            ...
            for k in range(1, self.refine_K + 1):
                proposals, pseudo_targets = self._proposal_cluster_learning(
                    boxes, scores, proposals, img_classes, suffix="_r{}".format(k)
                )

                predictions = self.box_predictor[k](box_features)

                losses_k = self.box_predictor[k].losses(predictions, proposals, pseudo_targets)
    ...
    def _proposal_cluster_learning(
        ...
        proposals = self.label_and_sample_proposals(
            proposals, pseudo_targets, is_PCL=True, suffix=suffix
        )
    ...
    def _get_cluster_centers(
        ...
        for (
            ...
            scores_per_image_cp = scores_per_image.clone().clamp_(self.epsilon, 1 - self.epsilon)

            for i, cls in enumerate(img_classes_per_image.reshape(-1, 1)):
                ...
                top_ranking_idxs = self._get_top_ranking_proposals(scores_per_class.reshape(-1, 1))
                top_ranking_boxes = boxes_per_class[top_ranking_idxs]
                top_ranking_scores = scores_per_class[top_ranking_idxs]

                graph = self._build_graph(top_ranking_boxes)
                ...
                while True:
                    ...
                    if remain_count <= self.min_remain_count:
                        break
    ...
    def _get_top_ranking_proposals(
        ...
        n_clusters = min(self.kmeans_num_clusters, scores_per_class_np.shape[0])
        kmeans = KMeans(n_clusters=n_clusters, random_state=self.kmeans_seed).fit(scores_per_class_np)
        ...
        if len(top_ranking_idxs) == 0:
            top_ranking_idxs = np.array([np.argmax(scores_per_class)])
    ...
    def _build_graph(
        ...
        return match_quality_matrix > self.graph_iou_threshold
